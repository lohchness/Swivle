...
func _ready():
	...
	for i in range(6):
		...
		newkey.set_base_position(Vector2(x_offset + (i * winsize.x / 6), winsize.y / 2))
		...
		#newkey.is_removed.connect(Callable(self, "remove_key"))
		#newkey.connect("select_signal", key_select_signal())
		#newkey.select_signal.connect(key_selected.bind())

func _process(delta):
	# Draw keys on screen
	#for i in range(len(keys)):
		#keys[i].position = Vector2(x_offset + (i * winsize.x / 6), winsize.y / 2)
		#
...
#func remove_key(number: int):
	#print("Removing key at " + str(number))
	#keys.remove_at(number)
...
func get_hand_string(from : int, to : int) -> String:
	print("Getting hand from indexes: " + str(from) + " to " + str(to))
...
func update_hand_positions():
	...
	for i in range(len(keys)):
		print(i)
		keys[i].set_base_position(Vector2(x_offset + (i * winsize.x / 6), winsize.y / 2))
